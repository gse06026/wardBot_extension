...
**Analogy-based explanation:**`;
  ...
  function showAnalogyResult(analogy) {
    ...
    // Use safe DOM manipulation to handle newlines instead of innerHTML
    const textWithLineBreaks = analogy.split('\n');
    content.textContent = ''; // Clear any existing content
    textWithLineBreaks.forEach((text, index) => {
      if (index > 0) {
        content.appendChild(document.createElement('br')); // Add line break
      }
      content.appendChild(document.createTextNode(text)); // Add text safely
    });
